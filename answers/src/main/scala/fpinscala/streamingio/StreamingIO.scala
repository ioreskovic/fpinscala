...
object GeneralizedStreamTransducers {
  ...
  object Process {
    ...
    def emitT[I, I2, O](h: O,
                        tl: Tee[I, I2, O] = haltT[I, I2, O]): Tee[I, I2, O] =
      emit(h, tl)
    ...
    val convertMultisink2: Process[IO, Unit] = (for {
      ...
      _ <- lines(file)
            ...
            .map(line => fahrenheitToCelsius(line.toDouble))
            .filter(_ > 0)
            . // ignore below zero temperatures
            map(_ toString)
            .to(fileW(file + ".celsius"))
    } yield ()) drain
  }
...
object ProcessTest extends App {
  ...
  val p2 = eval(IO { println("cleanup"); 2 }).onHalt {
    case Kill =>
      println { "cleanup was killed, instead of bring run" }; Halt(Kill)
    case e => Halt(e)
  }

  println {
    Process.runLog { p2.onComplete(p2).onComplete(p2).take(1).take(1) }
  }
